...
export const counterSlice = createSlice({
  ...
  reducers: {
    ...
    addProduct:  (state,action)=>{
        const newItem = state.item.find((ele)=>ele.itemid===action.payload.id);
        if(!newItem){
            state.item.push({itemid:action.payload.id,quantity:1});
        }
        else{
            const index = state.item.findIndex((ele)=>ele.itemid===newItem.itemid);
    ...
    removeProduct: (state,action)=>{
        const index = state.item.findIndex((ele)=>ele.itemid===action.payload.id);
...
export const { setCount,setproPrice,setcheckPrice,setshipPrice,setUserId,setItem,setAuth,addProduct,removeProduct,increaseQuantity,decreaseQuantity } = counterSlice.actions
